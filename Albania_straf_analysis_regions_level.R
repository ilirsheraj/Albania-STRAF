...
# This script takes allelle file and associated metadata (City of origin and Region)
# And returns a set of statistics and plots, among them AMOVA
library(tidyverse)
library(ape)
library(forensicpopdata)
# devtools::install_github("agouy/straf")
library(straf)
library(adegenet)
library(hierfstat)
library(pegas)
...
# Create the population index based on cities and regions
...
af_plot <- ggplot(af_df, aes(x = as.numeric(Allele), y = Freq)) +
...
ggsave("Allele_Frequency_Histrograms_per_Locus.pdf", 
       plot = af_plot, 
       width = 6, 
       height = 6, 
       units = "in",
...
# -----------------------------------------------------
# Part 5: Further EDA: PCA & MDS
# -----------------------------------------------------
# Build a PCA
gen_scaled <- scaleGen(gen,
                       center = TRUE,
                       scale = FALSE,
                       NA.method = "mean")

pca <- prcomp(gen_scaled, center = FALSE, scale. = FALSE)
...
ggsave("PCA_region_colored.jpeg", 
       plot = pca_plot, 
       width = 4, 
       height = 3, 
       units = "in",
...
pca_df <- tibble(PC = seq_along(pca$sdev),
  Variance = pca$sdev^2 / sum(pca$sdev^2))

scree_plot <- ggplot(pca_df, aes(PC, Variance)) +
  geom_line() +
  geom_point(size = 1) +
...
ggsave("Albanian_Population_Scree_Plot.pdf", 
       plot = scree_plot, 
       width = 4, 
       height = 3, 
       units = "in",
...
# MDS
# Repeat lengths for each locus
replen <- rep(4, nLoc(gen))
names(replen) <- locNames(gen)

# D22S1045 is trinucleotide
replen["D22S1045"] <- 3

# Calculate Bruvo genetic distances between individuals
bruvo_d <- bruvo.dist(gen, replen = replen)

mds <- cmdscale(as.dist(bruvo_d),
                k = 2,
                eig = TRUE)

# Coordinates + population/region information
mds_df <- data.frame(
  MDS1 = mds$points[, 1],
  MDS2 = mds$points[, 2],
  pop  = as.factor(pop(gen)))

# Percentage represented by each MDS axis
eig_pos <- mds$eig[mds$eig > 0]

mds_pve <- round(100 * eig_pos / sum(eig_pos), 2)

# Plot
mds_plot <- ggplot(mds_df, aes(MDS1, MDS2, color = pop)) +
  geom_point(size = 2, alpha = 0.7) +
  scale_color_manual(values = pop_cols, name = "Region") +
  labs(
    title = "",
    x = paste0("MDS1 (", mds_pve[1], "%)"),
    y = paste0("MDS2 (", mds_pve[2], "%)")) +
  theme_classic()

ggsave("MDS_region_colored.pdf", 
       plot = mds_plot, 
       width = 5, 
       height = 3, 
       units = "in",
...
tsne_plot <- ggplot(tsne_df, aes(Dim1, Dim2, color = Region)) +
  geom_point(size = 2.5) +
  scale_color_manual(values = pop_cols, name = "Region") +
  labs(
    # title = paste0("Albanian Population t-SNE (perplexity=", perp, ")"),
    title = "",
    x = "t-SNE 1", 
    y = "t-SNE 2") +
  theme_classic() +
...
ggsave("TSNE_region_colored.pdf", 
       plot = tsne_plot, 
       width = 5, 
       height = 4, 
       units = "in",
...
umap_plot <- ggplot(umap_df, aes(UMAP1, UMAP2, color = Region)) +
  geom_point(size = 2.5) +
  scale_color_manual(values = pop_cols, name = "Region") +
  labs(
    # title = "Albanian Population UMAP",
    title = "",
    x = "UMAP 1", 
    y = "UMAP 2") +
  theme_classic() +
...
ggsave("UMAP_region_colored.pdf", 
       plot = umap_plot, 
       width = 5, 
       height = 4, 
       units = "in",
...
# --------------------------------------------------------
# Part 7: Unsupervised Learning: Discriminant Analysis PCA
# --------------------------------------------------------
# Note: This was an important exploration before STRUCTURE Analysis, now its 
# sort of redundant, but i'm keeping it here because writing this code took me
# days of work and great headache!
set.seed(42)

# We are keen on K=3, so let's try k=2-6
Ks <- 2:6
...
# Sharp secrease at k=3: 7142.509 3418.972 2464.820 2093.831 1830.692

# Do Cross Validation and see how it performs
set.seed(42)
xval <- xvalDapc(
  gen,
...
# If you want, you can use Ind and see membership of each individual in clusters
write.csv(dapc_membership, 
          file = paste0(out_dir, "/ld_on_pca_unsupervised_clustering.csv"))
...
pdf(file = paste0(plot_dir, "/Albanian_Population_DA_PC_k3.pdf"), width = 5, height = 4)
scatter(dapc1)
dev.off()
...
# Notes: Albanian STR variation shows no discrete regional genetic structure; 
# observed differentiation is weak, diffuse, and consistent with high gene flow and panmixia.

cluster_regions <- inner_join(region_meta, dapc_membership, by = c("ind" = "Ind"))

table(cluster_regions$City, cluster_regions$Cluster)

clust_reg <- ggplot(cluster_regions, aes(x = City, fill = factor(Cluster))) +
...
ggsave("Cluster_per_city_barplot.pdf", 
       plot = clust_reg, 
       width = 6, 
       height = 5, 
       units = "in",
...
# ------------------------------------------------------
# Part 8: Weir & Cockerham’s FST
# ------------------------------------------------------
# How genetically differentiated are each pair of populations, 
# relative to the total genetic variance?
hf <- genind2hierfstat(gen)
fst_score <- pairwise.WCfst(hf)
fst_score

write.csv(fst_score, file = paste0(out_dir, "/fst_population_differences.csv"))

# Negatives are useless, so lets remove them
fst2 <- fst_score
fst2[fst2 < 0] <- 0
range(fst_score, na.rm=TRUE)
...
# ------------------------------------------------------
# Part 9: Hardy-Weinberg Equilibrium per Locus
# ------------------------------------------------------
...
# -----------------------------------------------------
# Part 10: Distance Tree
# ----------------------------------------------------
# Create Distance Tree to compare regions
...
region_nei <- ggplot(dfD, aes(x = Locus_2, y = Locus_1, fill = NeiD)) +
  geom_tile(color="white", linewidth=0.3) +
  # scale_fill_viridis_c(option = "magma", direction = -1) +
  theme_classic() +
  theme(
...
ggsave("NeI_Distance_Btw_Regions.pdf", 
       plot = region_nei, 
       width = 3.5, 
       height = 3.5, 
       units = "in",
...
nei_boot <- ggplot(boot_long, aes(Pair, NeiD, fill = Pair)) +
...
ggsave("Bootstrap_Nei_Dist_Violin.pdf", 
       plot = nei_boot, 
       width = 5, 
       height = 5, 
       units = "in",
...
# Generate a graph with confidence intervals: Violin is better, but old school likes it
sum_df <- boot_long %>%
  group_by(Pair) %>%
  summarise(
...
nei_boot_ci <- ggplot(sum_df, aes(Pair, median)) +
  geom_point(size = 2) +
...
ggsave("Bootstrap_CI_Nei_Dist.pdf", 
       plot = nei_boot_ci, 
       width = 5, 
       height = 5, 
       units = "in",
...
ggsave("Bootstrap_per_locus_nei_drivers_heatmap.pdf", 
       plot = driver_loci, 
       width = 5, 
       height = 5, 
       units = "in",
